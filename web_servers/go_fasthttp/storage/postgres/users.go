...
func (repo PostgresUserRepo) GetUserById(user_id int) (*storage.User, error) {
	...
	var row = repo.db.QueryRow(context.Background(), GET_USER_BY_ID_QUERY, user_id)
...
func (repo PostgresUserRepo) UpdateUser(user_id int, userUpdate storage.UserUpdate) (*storage.User, error) {
	var user storage.User
	var row = repo.db.QueryRow(context.Background(), UPDATE_USER_QUERY, user_id, userUpdate.Username, userUpdate.Email)
	err := pgxscan.NewScanner(row).Scan(&user.UserID, &user.Username, &user.Email, &user.IsArchived, &user.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func CreatePostgresUserRepo() (*PostgresUserRepo, error) {
	db, err := pgxpool.New(context.Background(), "postgresql://postgres:pass@localhost:15432/webservers_bench?pool_max_conns=80")
	if err != nil {
		return nil, err
	}
