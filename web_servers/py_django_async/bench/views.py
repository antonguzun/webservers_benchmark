...
class UserView(View):
    async def get(self, request: HttpRequest, user_id: int):
        if request.headers.get("token") != "hardcoded_token":
            return JsonResponse(data={"description": "unauthorized"}, status=403)
        logger.info(f"getting user with id {user_id}")
        try:
            user = await sync_to_async(get_user, thread_sensitive=True)(user_id=user_id)
            return JsonResponse(
                data=dict(
    ...
    async def patch(self, request: HttpRequest, user_id: int):
        if request.headers.get("token") != "hardcoded_token":
            return JsonResponse(data={"description": "unauthorized"}, status=403)
        logger.info(f"try to update with id {user_id}")
        data = dict(json.loads(request.body.decode()))
        user = await sync_to_async(update_user, thread_sensitive=True)(
...
async def to_json(request: HttpRequest):
    if request.headers.get("token") != "hardcoded_token":
        return JsonResponse(data={"description": "unauthorized"}, status=403)
...
async def plain(request: HttpRequest):
    if request.headers.get("token") != "hardcoded_token":
        return JsonResponse(data={"description": "unauthorized"}, status=403)
    param1 = request.GET.get("param1")
    param2 = request.GET.get("param2")
